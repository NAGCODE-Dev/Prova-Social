...
class _PreferencesAttemptQueueStorage implements AttemptQueueStorage {
  ...
  Future<void> write(String value) async {
    ...
    if (!saved) throw StateError('Não foi possível salvar a entrega no aparelho.');
  }
}
...
class AttemptQueueStore {
  ...
  Future<Map<String, dynamic>> _read() async {
    ...
    if (raw == null) return {'pending': <dynamic>[], 'completed': <String, dynamic>{}};
  ...
  Future<void> enqueue(AttemptSubmission submission) => _locked(() async {
        ...
        final duplicateIndex = pending.indexWhere((item) =>
  ...
  Future<void> update(PendingAttempt value) => _locked(() async {
        final data = await _read();
        final pending = List<dynamic>.from(data['pending'] as List? ?? const []);
        final index = pending.indexWhere((item) =>
            (item as Map)['clientAttemptId'] ==
            value.submission.clientAttemptId);
  ...
  Future<Map<String, ExamResult>> completed() => _locked(() async {
        ...
        return completed.map((key, value) => MapEntry(
              key,
...
class AttemptSyncService {
  AttemptSyncService({AttemptQueueStore? store, AttemptSubmitter? submitter})
      : store = store ?? AttemptQueueStore(),
        _submitter = submitter ?? AttemptRepository();
  ...
  Future<void> saveForSync(AttemptSubmission submission) =>
  ...
  Future<AttemptSyncOutcome> sync(
    ...
    final operation = _globalSyncTail.then(
      (_) => _sync(clientAttemptId, ignoreSchedule: ignoreSchedule),
    );
  ...
  Future<AttemptSyncOutcome> _sync(
    ...
    try {
      final result = await _submitter.submit(pending.submission);
...
AttemptSubmission _submissionFromJson(Map<String, dynamic> json) =>
    AttemptSubmission(
      ...
      answers: Map<String, dynamic>.from(json['answers'] as Map).map(
...
Exam _examFromJson(Map<String, dynamic> json) => Exam(
      ...
      questions: List<dynamic>.from(json['questions'] as List).map((item) {
...
Map<String, dynamic> _resultToJson(ExamResult result) => {
      ..._submissionToJson(AttemptSubmission(
        clientAttemptId: '',
        exam: result.exam,
        answers: result.answers,
        markedForReview: result.markedForReview,
