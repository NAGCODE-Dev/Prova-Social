...
void main() {
  test('respostas rápidas são gravadas em ordem e a última é restaurada', () async {
    final storage = ControlledStorage();
    final store = AttemptDraftStore(storage: storage);
    final mutable = draft(0).answers;
    final first = store.save('exam', AttemptDraft(
      answers: mutable, review: const {}, current: 0, elapsedSeconds: 3,
    ));
    mutable[0] = 1;
  ...
  test('flush aguarda a última alteração mesmo se entrar durante a espera', () async {
    final storage = ControlledStorage();
    final store = AttemptDraftStore(storage: storage);
    ...
    await tick();
    var completed = false;
    unawaited(flushing.then<void>((_) {
      completed = true;
    }));
    await tick();
  ...
  test('falha mantém o último estado em memória e permite tentar novamente', () async {
