-- Compatibility with the deployed legacy schema, without deleting any rows.
-- Both old and new RPCs remain available. Only RPCs write attempts/results.
...
revoke all on table public.attempts, public.attempt_answers
  from public, anon, authenticated;
grant select on table public.attempts, public.attempt_answers to authenticated;
...
-- Reinstall the exact idempotent overload, also on deployments whose migration
-- history differs from the repository. Historical migrations stay unchanged.
